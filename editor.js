...
function draw () {
  ...
  if (mode == '1') {
    let faceShape = map(s1, 0, 100, 0, 1);
    let faceType = map(s2, 0, 100, 0, 1);
    let waviness = map(s3, 0, 100, 0, .5);
    let xIncrement = int(map(s4, 0, 100, 1, 8));
    let eyeType = map(s5, 0, 100, 0, 1);
    let faceHeight = map(s6, 0, 100, 0, -5);
    let backgroundCol = map(s7, 0, 100, 0, 1);
    drawFace1(waviness, faceShape, faceType, xIncrement, eyeType, faceHeight, backgroundCol);
  }
